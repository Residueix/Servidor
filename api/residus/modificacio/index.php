        ...
        if(isset($_POST["id_usuari"])){
            ...
            if(isset($_POST["token"])){
                ...
                if($tokenActiu == true){
                    ...
                    if(isset($_POST["descripcio"])){ $descripcio = $_POST["descripcio"]; if(trim($descripcio)==""){$descripcio=null;}  }else{ $descripcio = null; }
                    if(isset($_POST["valor"])){ $valor = $_POST["valor"]; if(trim($valor)==""){$valor=null;} }else{ $valor = null; }
                    if(isset($_POST["actiu"])){ $actiu = $_POST["actiu"]; }else{ $actiu = null; }
                    
                    if($permis == "1" || $permis == "2"){
                        
                        if((!is_null($id)) && (!is_null($nom)) && (!is_null($tipus)) && (!is_null($descripcio)) && (!is_null($valor)) && (!is_null($actiu))){
                            ...
                            if($imatge){
                                ...
                                if(move_uploaded_file($_FILES["imatge"]["tmp_name"], "/opt/lampp/htdocs/residueix/img/residus/" . $nomFinalImatge)){
                                    echo $db->modificarResidu($id,$tipus,$nom,$nomFinalImatge,$descripcio,$valor,$actiu);
                                }else{
                                    echo $errors["64"];
                                }
                            }else {
                                echo $db->modificarResidu($id,$tipus,$nom,null,$descripcio,$valor,$actiu);
                            }
